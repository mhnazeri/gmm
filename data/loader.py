...
def create_feature_matrix(file):
    ...
    ego = []
    agents = np.zeros((len(datum[-1]), 560), dtype=np.float64)
    ...
    for key, start, stop in appears:
        for i in range(stop - start):
            ...
            agents[num, (start * 14) + (i * 14): (start + 1) * 14 + (i * 14)] = np.array(agent_data, dtype=np.float64)
        num += 1
    ...
    return np.array(ego, dtype=np.float64).reshape(1, 560), agents


ego, agents = create_feature_matrix("exported_json_data/scene-1100.json")
print(agents.shape)
print(agents[0])