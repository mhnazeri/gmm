...
summary_writer_validation = SummaryWriter(
    os.path.join(DIRECTORIES["log"], "validation_loss")
)
summary_writer_generator = SummaryWriter(os.path.join(DIRECTORIES["log"], "generator"))
summary_writer_discriminator = SummaryWriter(
    os.path.join(DIRECTORIES["log"], "discriminator")
)
...
def get_cae():
    ...
    cae_encoder, cae_decoder = make_cae(
...
def main():
    ...
    d_optimizer = torch.optim.AdamW(
        d.parameters(), lr=float(DISCRIMINATOR["learning_rate"]), betas=(0.5, 0.999)
    )
    g_optimizer = torch.optim.AdamW(
        g.parameters(), lr=float(GENERATOR["learning_rate"]), betas=(0.5, 0.999)
    )
    ...
    for epoch in range(start_epoch, start_epoch + int(TRAINING["epochs"])):
        ...
        for batch in data_loader:
            ...
            fake_labels = torch.ones(batch["past"].shape[1]).to(
                device
            ) * random.uniform(0, 0.2)
            true_labels = torch.ones(batch["past"].shape[1]).to(
                device
            ...
            while d_steps_left > 0:
                ...
                real_predictions = d(
                    torch.cat([batch["rel_past"][:, :, :2], batch["rel_future"]], dim=0)
                )
                real_loss = bce_loss(real_predictions, true_labels)

                fake_traj = g(batch["past"], batch["rel_past"], batch["motion"])
                fake_prediction = d(
                    torch.cat([batch["rel_past"][:, :, :2], fake_traj.detach()], dim=0)
                )
                ...
                if clipping_threshold_d > 0:
                    nn.utils.clip_grad_norm_(d.parameters(), clipping_threshold_d)
            ...
            fake_prediction = d(
                torch.cat([batch["rel_past"][:, :, :2], fake_traj], dim=0)
            )
        ...
        with torch.no_grad():
            ...
            t1 = time.time()
            fake_traj = g(
                batch["past"][:, :max_agents, :],
                batch["rel_past"][:, :max_agents, :],
            ...
            ADE_loss = displacement_error(
                fake_traj, batch["future"][:, :max_agents, :]
            )[0].item()
            FDE_loss = final_displacement_error(
                fake_traj[-1], batch["future"][:, :max_agents, :]
            )[0].item()
            ...
            for _ in range(int(TRAINING["k_samples"])):
                ...
                fake_traj = relative_to_abs(
                    fake_traj, batch["past"][-1, :max_agents, :2]
                )
                msd.append(
                    msd_error(fake_traj, batch["future"][:, :max_agents, :])[0].item()
                )
        ...
        ##########################################################################################
        if (
            (ADE_loss <= best_ADE_loss and epoch > int(TRAINING["ignore_first_epochs"]))
            or (epoch + 1) % int(TRAINING["save_every_d_steps"]) == 0
            or (epoch + 1) == start_epoch + int(TRAINING["epochs"])
        ):
            ...
            if ADE_loss <= best_ADE_loss and epoch > int(
                TRAINING["ignore_first_epochs"]
            ):
...
def validate(g, device=None):
    ...
    if not g:
        ...
        g = TrajectoryGenerator(
            ...
            input_size=int(TRAINING["input_size"]),
            output_size=int(GENERATOR["output_size"]),
            decoder_mlp_structure=convert_str_to_list(
                GENERATOR["decoder_h2p_structure"]
            ),
            decoder_mlp_activation=str(GENERATOR["decoder_h2p_activation"]),
            dropout=float(GENERATOR["dropout"]),
            fusion_pool_dim=int(GENERATOR["fusion_pool_dim"]),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train", default=True, action="store_true", help="train the model or test?"
    )
